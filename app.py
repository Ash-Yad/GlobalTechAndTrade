...
# ==========================================
# ROBUST FIX FOR RENDER DEPLOYMENT
# ==========================================
...
# CRITICAL FIX: Force Flask to use the instance folder path to find templates
# This ensures Flask looks in the directory where app.py is located.
# We manually set the template folder to the absolute path of './templates' relative to this file.
...
if not app.debug:
    ...
    # Log the path so we can verify in Render logs
    app.logger.info(f'GlobalTech&Trade startup. Template path set to: {template_folder_path}')
...
def index():
    """Home page route"""
    # Featured services for homepage hero section
    featured_services = [
        {
